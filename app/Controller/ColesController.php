...
class ColesController extends AppController
{
    public $name = 'Coles';
    public $uses = array("Product","ProductCategory","ProductSubCategory","ProductPrice","Postcode","ProductDiscount","ProductItem","WoolworthsProduct","ColesProduct","ColesPrice","WoolworthsPrice");
    ...
    function shophead_index(){
        ...
        $this->set('title','Coles Management');
        $conditions=array("ColesProduct.is_deleted"=>0);
        $this->paginate = array(
            'recursive' => 0,
            'limit' => LIMIT,
            'conditions' => $conditions,
            'order' => array(
                'ColesProduct.updated' => 'Desc'
            )
        );
    ...
    function shophead_edit($product_id = "")
    {
        ...
        $data = $this->ColesProduct->find('first', array('conditions' => array('ColesProduct.id' => $id)));
        $productpricelist=$this->ColesPrice->find('all',array('conditions'=>array('ColesPrice.coles_product_id'=>$id)));
        $this->set('productpricelist', $productpricelist);        
        if (!empty($data)) {
            $sub_categories_list = $this->ProductSubCategory->find('list', array('conditions' => array('ProductSubCategory.id' => $data['ColesProduct']['subcategory_id']), 'fields' => array('ProductSubCategory.id', 'ProductSubCategory.name')));
                $this->set('sub_categories_list', $sub_categories_list);
                
            $item_list = $this->ProductItem->find('list', array('conditions' => array('ProductItem.id' => $data['ColesProduct']['product_item_id']), 'fields' => array('ProductItem.id', 'ProductItem.name')));
